...
private[scalatest] class CoverageHtmlReporter extends ResourcefulReporter {
  ...
  private def makeSuiteFile(suiteResult: SuiteResult): Unit = {
    ...
    val pw = new PrintWriter(
      new OutputStreamWriter(
        new BufferedOutputStream(new FileOutputStream(new File(targetDir, name + ".html")), BufferSize),
        "UTF-8"
      )
    )
    try {
      ...
      suiteResult.eventList.map { e =>
        e match {
          case ScopeOpened(ordinal, message, nameInfo, formatter, location, payload, threadName, timeStamp) =>
            ...
            val stringToPrint =
              stringToPrintWhenNoError(Resources.scopeOpened _, formatter, nameInfo.suiteName, nameInfo.testName)
            stringToPrint match {
              case Some(string) =>
          ...
          case ScopePending(ordinal, message, nameInfo, formatter, location, payload, threadName, timeStamp) =>
            val testNameInfo = nameInfo.testName
            val stringToPrint =
              stringToPrintWhenNoError(Resources.scopePending _, formatter, nameInfo.suiteName, nameInfo.testName)
            stringToPrint match {
              case Some(string) =>
                val elementId = generateElementId
                scope(elementId, string, getIndentLevel(formatter) + 1)
          ...
          case TestSucceeded(
            ...
            val stringToPrint =
              stringToPrintWhenNoError(Resources.testSucceeded _, formatter, suiteName, Some(testName), duration)
  ...
  private def makeIndexFile(
    ...
    val pw = new PrintWriter(
      new OutputStreamWriter(
        new BufferedOutputStream(new FileOutputStream(new File(targetDir, "index.html")), BufferSize),
        "UTF-8"
      )
    )
    try {
  ...
  private def setBit(
    ...
    tagMap: collection.mutable.HashMap[String, Int],
    bit:    Int
  ): Unit = {
