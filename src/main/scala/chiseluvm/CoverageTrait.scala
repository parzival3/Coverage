...
trait CoverageTrait extends ChiselScalatestTester {
  ...
  /**
    * TODO: The SimpleCoverageReport of jgenhtml is not very flexible, we should consider to extend it with another class
    * TODO: Get the actual coverage percentage
    * Coverage report generated by jgenhtml
    * @param path path of the complete report
    * @return the coverage percentage
    */
  def CreateCoverageReport(path: String): Float = {
    val file = new Array[String](1)
    file(0) = path + "/total/output.info"
    // Create a new Config for the SimpleCoverageReport and set as command line the output directory
    val config = new Config()
    config.initializeUserPrefs(List("--output-directory", path).toArray)
    config.setHtmlOnly(true)
    config.setBranchCoverage(false)
    config.setFunctionCoverage(false)
    // Set the configuration to the SimpleCoverageReport singleton
    val coverageReport = new CoverageReport(file, config)
  ...
  def createVerilogReport(): Unit = {
    ...
    CreateCoverageReport(coverageFolder)
    println(s"Coverage saved in $coverageFolder/total/VerilatorCoverage.html")
